...
function init() {
  ...
  if (storedSaveType) {
    document.querySelector('.starting.load.button .save-type-label').insertAdjacentText('beforeend', storedSaveType);
  ...
  themeToggle.addEventListener('click', () => {
    ...
    localStorage.setItem('sakusorter-theme', next);
...
function start() {
  ...
  options.forEach(opt => {
    if ('sub' in opt) {
      // Check if any sub-checkbox is checked for this group
      const hasAnyChecked = opt.sub.some((val, idx) => 
        document.getElementById(`cb-${opt.key}-${idx}`).checked
      );
      if (!hasAnyChecked) optTaken.push(false);
      else {
...
function result(imageNum = 7) {
  ...
  const kami = (char, num) => {
	...
	return `<div class="col-12 mb-3"><div class="row justify-content-center"><div class="result kami col-6 col-md-4 d-flex"><div class="flex-fill result-mcontainer result-kami-card p-2 text-center"><div class="result-crown mb-1" style="font-size:1.4em;">&#128081;</div><img class="w-100 mx-auto rounded-lg mb-2" style="border-radius:12px !important;" src="${char.img}"><div class="font-weight-bold text-break result-kami-name" title="${charTooltip}" style="font-size:0.82em;">${num}. ${charName}</div></div></div></div></div>`;
  }
  ...
  const imgRes = (char, num) => 
  {
    ...
    const charTooltip = char.name !== charName ? char.name : '';
	return `<div class="result image col-4 pl-0 pr-1 pb-2 d-flex"><div class="flex-fill result-mcontainer p-2 text-center" style="border-radius:12px !important;"><img class="w-100 mx-auto rounded-lg mb-1" style="border-radius:10px !important;" src="${char.img}"><div class="text-break text-center result-img-name" title="${charTooltip}" style="font-size:0.82em;font-weight:600;">${num}. ${charName}</div></div></div>`;
  }
  ...
  const res = (char, num) =>
  {
	const charName = reduceTextWidth(char.name, 'Arial 12px', 500);
	const charTooltip = char.name !== charName ? char.name : '';
	return `<div class="result col-4 px-1 pb-2 d-flex"><div class="flex-fill result-mcontainer p-2 align-items-center text-center" style="border-radius:10px !important;"><div class="text-break result-text-name" title="${charTooltip}" style="font-size:0.82em;font-weight:500;">${num}. ${charName}</div></div></div>`;
  }
  const timeStr = `Completed on ${new Date(timestamp + timeTaken).toLocaleDateString()} · Took ${msToReadableTime(timeTaken)} <a href="${location.protocol}//${sorterURL}" class="result-sort-again">Sort again?</a>`;
...
function generateTextList() {
  ...
  oWindow.document.write(`<!DOCTYPE html><html><head><title>Sakurazaka46 Sort Result</title><style>body{font-family:'Inter',sans-serif;padding:24px;background:#0e0e14;color:#f0e6ef;line-height:1.8;font-size:14px;}h2{color:#f472b6;margin-bottom:16px;font-size:18px;}@media(prefers-color-scheme:light){body{background:#f8f0f4;color:#1a1a2e;}h2{color:#db2777;}}</style></head><body><h2>Sakurazaka46 Sorter — Your Ranking</h2>${data}</body></html>`);
}
...
function populateOptions() {
  ...
  const options = dataSet[dataSetVersion].options;

  options.forEach(opt => {
    if ('sub' in opt) {
      const groupContainer = document.createElement('div');
      groupContainer.className = 'mt-3 card mx-auto sakurazaka-card';
      groupContainer.style.cssText = 'max-width: 480px; width: 92%;';

      const groupHeader = document.createElement('div');
      groupHeader.className = 'px-3 py-2 d-flex align-items-center';
      groupHeader.innerHTML = `
        <div class="sakurazaka flex-grow-1">
          <span class="font-weight-bold opt-group-name" style="font-size:0.85rem;">
            ${opt.name}
          </span>
        </div>
        <span class="header-icon rotate opt-toggle-icon" style="cursor:pointer;font-size:1.2rem;transition:transform 0.3s ease;">&#9650;</span>
      `;

      const itemListContainer = document.createElement('div');
      itemListContainer.className = 'item-list px-3 pb-2';

      opt.sub.forEach((subopt, subindex) => {
        const checkboxItem = document.createElement('div');
        checkboxItem.className = 'border-t py-2';
        checkboxItem.innerHTML = `
          <div class="ml-3 sakurazaka">
            <label class="checkbox-wrap">
              <input type="checkbox" class="sub-checkbox" data-group="${opt.key}" id="cb-${opt.key}-${subindex}" ${subopt.checked !== undefined ? (subopt.checked ? 'checked' : '') : (opt.checked ? 'checked' : '')}>
              <span class="checkmark"></span>
              <span class="opt-sub-name" style="font-size:0.8rem;">${subopt.name}</span>
            </label>
          </div>
        `;
        itemListContainer.appendChild(checkboxItem);
      });

      groupContainer.appendChild(groupHeader);
      groupContainer.appendChild(itemListContainer);

      optList.appendChild(groupContainer);
    } else {
      const singleCheckboxContainer = document.createElement('div');
      singleCheckboxContainer.className = 'mt-3 card mx-auto sakurazaka-card';
      singleCheckboxContainer.style.cssText = 'max-width: 480px; width: 92%;';
      
      singleCheckboxContainer.innerHTML = `
        <div class="px-3 py-2 d-flex align-items-center">
          <div class="sakurazaka flex-grow-1">
            <label class="checkbox-wrap">
              <input type="checkbox" id="cb-${opt.key}" ${opt.checked ? 'checked' : ''}>
              <span class="checkmark"></span>
              <span class="font-weight-bold opt-group-name" style="font-size:0.85rem;">
                ${opt.name}
              </span>
      ...
      optList.appendChild(singleCheckboxContainer);
    }
  });
  ...
  document.querySelectorAll('.header-icon').forEach(icon => {
    icon.addEventListener('click', function() {
      ...
      const itemList = this.closest('.sakurazaka-card').querySelector('.item-list');
      if (itemList) {
