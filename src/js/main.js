...
function populateOptions() {
  ...
  options.forEach(opt => {
    if ('sub' in opt) {
      ...
      groupContainer.className = 'mt-5 card mx-auto v-card v-sheet theme--light elevation-0 sakurazaka-card';
      groupContainer.style.cssText = 'max-width: 480px; width: 90%;';

      const groupHeader = document.createElement('div');
      groupHeader.className = 'px-1 py-2 my-auto d-flex';
      groupHeader.innerHTML = `
        <div class="sakurazaka flex-grow-1 text-center">
          <label class="checkbox-wrap">
            <input type="checkbox" id="cbgroup-${opt.key}" ${opt.checked ? 'checked' : ''}>
            <span class="checkmark"></span>
            <span class="grey--text text--darken-1 text-subtitle-1 font-weight-bold">
              ${opt.name}
            </span>
          </label>
        </div>
        <span class="mdi mdi-chevron-up ml-auto my-auto text-h4 grey--text header-icon rotate"></span>
      `;

      const itemListContainer = document.createElement('div');
      itemListContainer.className = 'item-list px-3';

      opt.sub.forEach((subopt, subindex) => {
        const checkboxItem = document.createElement('div');
        checkboxItem.className = 'border-t py-2';
        checkboxItem.innerHTML = `
          <div class="ml-4 sakurazaka">
            <label class="checkbox-wrap">
      ...
      singleCheckboxContainer.className = 'mt-5 card mx-auto v-card v-sheet theme--light elevation-0 sakurazaka-card';
      singleCheckboxContainer.style.cssText = 'max-width: 480px; width: 90%;';
      
      singleCheckboxContainer.innerHTML = `
        <div class="px-1 py-2 my-auto d-flex">
          <div class="sakurazaka flex-grow-1 text-center">
            <label class="checkbox-wrap">
  ...
  document.querySelectorAll('[id^="cbgroup-"]').forEach(groupCheckbox => {
    groupCheckbox.addEventListener('change', function() {
      const group = this.id.replace('cbgroup-', '');
      const subCheckboxes = document.querySelectorAll(`.sub-checkbox[data-group="${group}"]`);
